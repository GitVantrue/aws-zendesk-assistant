...
def generate_html_report(json_file_path):
    ...
    Reference 코드의 generate_html_report 함수를 재사용
    
    Args:
        json_file_path (str): JSON 데이터 파일 경로
    
    Returns:
        str: 생성된 HTML 파일 경로 또는 None (실패시)
    """
    try:
        ...
        template_vars.update({
            'ta_available': ta_data.get('available', False),
            'ta_checks': ta_data.get('security_checks', 0),
            'ta_content': generate_ta_content(ta_data.get('results', [])),
        })
        
        # CloudTrail 데이터 처리
        ct_data = data.get('cloudtrail_events', {})
        template_vars.update({
            'ct_total': ct_data.get('total_events', 0),
            'ct_content': generate_cloudtrail_content(ct_data.get('events', [])),
        })
        
        # CloudWatch 데이터 처리
        cw_data = data.get('cloudwatch', {})
        template_vars.update({
            'cw_total': cw_data.get('total_alarms', 0),
