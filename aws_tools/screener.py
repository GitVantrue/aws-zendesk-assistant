...
def run_service_screener_sync(account_id, credentials=None, websocket=None, session_id=None):
    ...
    try:
        ...
        # ========================================
        # 스캔할 리전 설정 (Reference 코드와 동일)
        # ========================================
        scan_regions = ['ap-northeast-2', 'us-east-1']
        region_string = ','.join(scan_regions)
        
        print(f"[DEBUG] 스캔 대상 리전: {region_string}", flush=True)
        ...
        # Service Screener main.py 실행 (Reference 코드와 동일: --regions 옵션 사용)
        cmd = [
            'python3',
