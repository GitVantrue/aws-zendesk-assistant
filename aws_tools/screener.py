...
def run_service_screener_sync(account_id, credentials=None, websocket=None, session_id=None, question=""):
    ...
    try:
        ...
        # Slack 봇과 동일: adminlte/aws/{account_id}에 결과 저장
                ...
                try:
                    ...
                    if os.path.exists(account_result_dir):
                        ...
                        if index_html_path:
                            ...
                            print(f"[DEBUG] 추출 후에도 index.html을 찾을 수 없음", flush=True)
                            return {
                                "success": True,
                                "summary": f"📊 계정 {account_id} 스캔이 완료되었으나 index.html을 찾을 수 없습니다.",
                                "report_url": None,
                        ...
                        return {
                            ...
                            "summary": None,
                            "report_url": None,
                            "error": "output.zip 추출 후에도 결과 디렉터리를 찾을 수 없습니다."
                        }
                    ...
                    return {
                        ...
                        "summary": None,
                        "report_url": None,
                        "error": f"output.zip 추출 실패: {str(e)}"
                    }
            else:
                # output.zip도 없음 = 스캔 실패
