/**
 * AWS AgentCore Chatbot - Zendesk App (Proxy Server Version)
 * Use this version if you're running the proxy server
 */

// ============================================
// AgentCore Proxy Service
// ============================================

class AgentCoreProxyService {
  constructor(config) {
  ...
  async invokeAgent(inputText, ticketContext = null) {
    ...
    try {
      const response = await fetch(`${this.proxyUrl}/api/agent/invoke`, {
        method: 'POST',
        headers: {
      ...
      console.error('AgentCore proxy invocation failed:', error);
      throw error;
    }
  }
...
// ============================================
// Zendesk App Client (Proxy Version)
// ============================================
...
class ZendeskChatbotApp {
  ...
  async init() {
    try {
      ...
      // Get app settings and initialize AgentCore proxy service
      const settings = await this.getSettings();
      this.agentService = new AgentCoreProxyService({
        proxyUrl: settings.proxyServerUrl || 'http://localhost:3000',
      ...
      console.log('AWS AgentCore Chatbot (Proxy) initialized successfully');
    } catch (error) {
